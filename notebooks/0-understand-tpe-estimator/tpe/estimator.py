...
class TPE:
    ...
    def plot_try(self, try_index = -1, show_x_tries = True, show_EI = True, save=None, filename = "fig",show=True):
        """
        ...
        if show_x_tries:
            ax1.scatter(Dl, np.zeros_like(Dl), c="blue", marker="x", label="Intentos buenos")#"good tries")
            ax1.scatter(Dg, np.zeros_like(Dg), c="orange", marker="x", label="Intentos malos")#"bad tries")
        ax1.plot(x, f(x), "--", c="red", label="Función Objetiva")#"Objective Function")
        ax1.set_ylabel(r"$f(x)$", c="red")
        ax1.plot(x, yp*np.ones_like(x), "--", c="green", label=r"$y*$")
        delta_y = ax1.get_ylim()[1] - ax2.get_ylim()[0]
        ax1.annotate(r"$y*$", (x[-1], yp), 
                        (x[-1], yp + delta_y/20), c="green")
        ax1.legend()
